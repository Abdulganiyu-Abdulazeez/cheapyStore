<template>
    <div class="w-full h-full p-[2rem]">
        <div class="" v-if="details">
            <div class="w-full h-fit py-[2rem] text-[40px] font-bold bg-white flex items-center justify-center">
                <h1 class="text-center w-full">Jewelery</h1>
            </div>
            <div class="w-full grid grid-cols-4 gap-[2rem] max-[768px]:grid-cols-2 max-[425px]:grid-cols-1">
                <div class="products w-full h-full" v-for="cloths in store" :key="cloths">
                    <div class="cloth w-full h-[75vh] flex flex-col items-center" v-show="cloths.category === jewelery">
                        <div class="w-full h-[60%] bg-white rounded-t-[10px] font-bold gap-[1rem] flex flex-col items-center p-[1rem]">
                            ...
                            <img class="w-full h-[35vh]" :src=cloths.image :alt="jewelery" @click="detail(cloths)">
                        </div>
...
<script>
    ...
    export default defineComponent ({
        ...
        methods: {
            detail(cloths) {
                this.fate = false,
                cheapy().$state.fullDetails = true
                this.details= this.fate
                this.category= cloths.category
                cheapy().$state.category= cloths.category
                this.price= cloths.price
                cheapy().$state.price= cloths.price
                this.description= cloths.description
                cheapy().$state.description= cloths.description
                this.title= cloths.title
                cheapy().$state.title= cloths.title
                this.image= cloths.image
                cheapy().$state.image= cloths.image
            },
            counting1() {
            ...
            params(cloths) {
                this.$router.push({ name: 'mensClothingFullDetails', params: {title: cloths.title}})
            },
            change() {
                this.$router.back;
                this.details = true;
